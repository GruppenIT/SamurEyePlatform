...
@test "INFRA-05: tarball contains app/, bin/, wordlists/, install.sh, MANIFEST.json" {
    if [[ "${BATS_SKIP_NETWORK:-}" == "1" ]]; then skip "Network tests disabled"; fi
    [ -f "$TARBALL_PATH" ] || skip "Tarball not built yet (run test 1 first)"
...
@test "INFRA-05: tarball bin/ contains all 4 binary archives" {
    if [[ "${BATS_SKIP_NETWORK:-}" == "1" ]]; then skip "Network tests disabled"; fi
    [ -f "$TARBALL_PATH" ] || skip "Tarball not built yet"
    local workdir
    workdir="$BATS_TEST_TMPDIR/extract-test3"
    mkdir -p "$workdir"
...
# ── Test 4: SHA-256 of each binary matches binaries.json ─────────────────────
@test "INFRA-05: each binary in tarball passes SHA verification" {
    if [[ "${BATS_SKIP_NETWORK:-}" == "1" ]]; then skip "Network tests disabled"; fi
    [ -f "$TARBALL_PATH" ] || skip "Tarball not built yet"
    local workdir
    workdir="$BATS_TEST_TMPDIR/extract-test4"
    mkdir -p "$workdir"
    tar -xzf "$TARBALL_PATH" -C "$workdir"
...
@test "INFRA-05: tarball wordlists/ contains both wordlist files" {
    ...
    [ -f "$TARBALL_PATH" ] || skip "Tarball not built yet"
    local workdir
    workdir="$BATS_TEST_TMPDIR/extract-test5"
    mkdir -p "$workdir"
    tar -xzf "$TARBALL_PATH" -C "$workdir"
...
@test "INFRA-05: MANIFEST.json URLs rewritten to ./bin/" {
    ...
    [ -f "$TARBALL_PATH" ] || skip "Tarball not built yet"
    local workdir
    workdir="$BATS_TEST_TMPDIR/extract-test6"
    mkdir -p "$workdir"
    tar -xzf "$TARBALL_PATH" -C "$workdir"
    local manifest="$workdir/samureye-${TEST_TAG}/MANIFEST.json"
...
@test "INFRA-05: MANIFEST.json preserves original SHA-256 for katana" {
    ...
    [ -f "$TARBALL_PATH" ] || skip "Tarball not built yet"
    local workdir
    workdir="$BATS_TEST_TMPDIR/extract-test7"
    mkdir -p "$workdir"
    tar -xzf "$TARBALL_PATH" -C "$workdir"
    local manifest="$workdir/samureye-${TEST_TAG}/MANIFEST.json"
...
teardown() {
    # Remove test tarball after all tests complete
    rm -f "$TARBALL_PATH"
}
