...
export class DatabaseStorage implements IStorage {
  ...
  async createAsset(asset: InsertAsset, userId: string): Promise<Asset> {
    ...
    if (existing.length > 0) {
      log.info(`ℹ️  Ativo duplicado ignorado: ${asset.value} (tipo: ${asset.type})`);
      return existing[0];
    }
  ...
  async findThreatByCorrelationKey(correlationKey: string): Promise<Threat | undefined> {
    log.info(`🔍 FIND: Searching for correlationKey: "${correlationKey}" (length: ${correlationKey.length})`);
    ...
    log.info(`🔍 FIND: Found ${allThreats.length} threats with correlationKey: "${correlationKey}"`);
    
    if (allThreats.length > 1) {
      log.info(`⚠️ FIND: Multiple threats found with same correlationKey! Using the first one.`);
      log.info(`🔍 FIND: All threats: ${allThreats.map(t => `${t.id} (${t.status}, ${t.createdAt})`).join(', ')}`);
    }
    
    const threat = allThreats[0];
    
    if (threat) {
      log.info(`✅ FIND: Using threat ${threat.id} with status: ${threat.status} (created: ${threat.createdAt})`);
    } else {
      log.info(`❌ FIND: No threat found for key: "${correlationKey}"`);
    }
  ...
  async upsertThreat(threat: InsertThreat & { correlationKey: string; category: string; lastSeenAt?: Date }): Promise<{ threat: Threat; isNew: boolean }> {
    log.info(`📋 UPSERT: Processing threat with correlationKey: ${threat.correlationKey}`);
    log.info(`📋 UPSERT: Input threat details - title: ${threat.title}, jobId: ${threat.jobId}, status: ${threat.status || 'open'}`);
    ...
    if (existingThreat) {
      log.info(`🔍 UPSERT: Found existing threat ${existingThreat.id} with status: ${existingThreat.status}, jobId: ${existingThreat.jobId}`);
      log.info(`🔍 UPSERT: Existing threat created: ${existingThreat.createdAt}, updated: ${existingThreat.updatedAt}`);
    } else {
      log.info(`🆕 UPSERT: No existing threat found for correlationKey: ${threat.correlationKey}`);
      log.info(`🔍 UPSERT: Searching for threats with similar keys...`);
      ...
      log.info(`🔍 UPSERT: Found ${similarThreats.length} similar threats:`, JSON.stringify(similarThreats, null, 2));
    }
    ...
    if (existingThreat) {
      ...
      log.info(`🔄 UPSERT: Existing threat status is '${existingThreat.status}', shouldReactivate: ${shouldReactivate}`);
      ...
      return await db.transaction(async (tx) => {
        ...
        if (shouldReactivate) {
          ...
          log.info(`🔄 Cross-journey reactivation: threat ${existingThreat.id} from ${existingThreat.status} to open`);
        }
        ...
        if (shouldReactivate) {
          ...
          log.info(`📋 Created status history for cross-journey threat reactivation: ${existingThreat.id}`);
        }
      ...
      log.info(`🆕 UPSERT: Creating new threat with correlationKey: ${threat.correlationKey}`);
      ...
      try {
        ...
        log.info(`✅ UPSERT: Processed threat ${newThreat.id} via onConflict - status: ${newThreat.status}`);
        ...
        if ((error as any)?.code === '42P10') {
          log.info(`⚠️  UPSERT: onConflict not supported, falling back to simple insert`);
          const [newThreat] = await db
            .insert(threats)
            .values({
              ...threat,
          ...
          log.info(`✅ UPSERT: Created new threat ${newThreat.id} via fallback insert - status: ${newThreat.status}`);
          return { threat: newThreat, isNew: true };
        }
  ...
  async getJourneyCredentials(journeyId: string): Promise<(JourneyCredential & { credential: Credential })[]> {
    log.info(`🔍 [STORAGE] getJourneyCredentials called with journeyId: ${journeyId}`);
    const results = await db
      .select({
    ...
    log.info(`  → Drizzle returned ${results.length} rows with JOIN`);
    ...
    log.info(`  → Mapped with credentials:`, JSON.stringify(plainResults.map(r => ({
      ...r,
  ...
  async upsertHost(host: InsertHost): Promise<Host> {
    ...
    log.info(`🔍 [UPSERT] Attempting upsert for host: ${normalizedName}, IPs: ${(host.ips || []).join(', ')}`);
    ...
    if (!existingHost && host.ips && host.ips.length > 0) {
      log.info(`🔍 [UPSERT] Not found by name, searching by IP: ${host.ips[0]}`);
      existingHost = await this.findHostByTarget(normalizedName, host.ips[0]);
    }
    
    if (existingHost) {
      log.info(`✅ [UPSERT] Found existing host: ${existingHost.name} (ID: ${existingHost.id}), updating...`);
      ...
      const [updatedHost] = await db
        ...
        .returning();
      log.info(`✅ [UPSERT] Host updated: ${updatedHost.name} (IPs: ${updatedHost.ips.join(', ')}, Aliases: ${(updatedHost.aliases || []).join(', ')})`);
      return updatedHost;
    } else {
      log.info(`➕ [UPSERT] No existing host found, creating new: ${normalizedName}`);
      // Create new host
      const hostValues = {
        ...host,
        name: normalizedName,
      ...
      const [newHost] = await db
        ...
        .values(hostValues)
        .returning();
      log.info(`✅ [UPSERT] New host created: ${newHost.name} (ID: ${newHost.id})`);
      return newHost;
    }
  }
  ...
  async getAuditLog(limit = 100): Promise<any[]> {
    ...
    const enrichedLogs = await Promise.all(logs.map(async (log) => {
      ...
      try {
        ...
        log.info(`Could not fetch details for ${log.objectType} ${log.objectId}`);
      }
  ...
  async initializeDatabaseStructure(): Promise<void> {
    try {
      log.info('🔧 Verificando estrutura do banco de dados...');
      ...
      log.info(`🔍 Índice único de correlation_key: ${hasUniqueIndex ? 'EXISTE' : 'NÃO EXISTE'}`);
      
      if (!hasUniqueIndex) {
        log.info('🔧 Criando estrutura de prevenção de duplicatas...');
        
        // Step 1: Consolidate existing duplicates
        log.info('📋 Consolidando duplicatas existentes...');
        await this.consolidateDuplicateThreats();
        
        // Step 2: Create unique index
        log.info('🏗️  Criando índice único para correlation_key...');
        ...
        log.info('✅ Índice único criado com sucesso!');
        log.info('🎉 Sistema de prevenção de duplicatas ativo!');
      } else {
        log.info('✅ Estrutura do banco de dados já está atualizada');
      }
      
    } catch (error) {
      log.error('❌ Erro na inicialização do banco:', error);
  ...
  private async ensureSystemUserExists(): Promise<void> {
    try {
      ...
      if (existingUsers.length === 0) {
        log.info('🤖 Criando usuário sistema...');
        ...
        log.info('✅ Usuário sistema criado com sucesso!');
      } else {
        log.info('✅ Usuário sistema já existe');
      }
      ...
      if (verifyUser.length === 0) {
        log.error('❌ Usuário sistema não foi criado corretamente!');
        throw new Error('Failed to create system user - this will cause FK violations');
      }
      
    } catch (error) {
      log.error('❌ Erro ao verificar/criar usuário sistema:', error);
  ...
  private async consolidateDuplicateThreats(): Promise<void> {
    try {
      ...
      log.info(`📋 Encontradas ${duplicates?.length || 0} chaves com duplicatas`);
      
      // Consolidate each group of duplicates
      let totalRemoved = 0;
      
      if (!duplicates || duplicates.length === 0) {
        log.info('📋 Nenhuma duplicata encontrada para consolidar');
        return;
      }
      ...
      for (const duplicate of duplicates) {
        ...
        if (removeIds.length > 0) {
          ...
          log.info(`🗑️  Removidas ${removeIds.length} duplicatas de ${(duplicate as any).correlation_key}`);
        }
      }
      
      log.info(`✅ Consolidação concluída: ${totalRemoved} duplicatas removidas`);
      
    } catch (error) {
      log.error('❌ Erro na consolidação de duplicatas:', error);
      throw error;
    }
  }
